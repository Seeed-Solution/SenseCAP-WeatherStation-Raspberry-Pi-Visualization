...
var groundMaterialxx = new THREE.MeshLambertMaterial({map: groundTexture});
...
if (false) {
    ...
    var material = new THREE.MeshPhongMaterial({color: 0x003300});
    var mesh = new THREE.Mesh(geometry, material);
    mesh.castShadow = true;
    mesh.receiveShadow = true;
    mesh.position.y = 3;
...
var grassMaterial = new THREE.RawShaderMaterial({
    uniforms: {
        ...
        halfxx: {type: 'float', value: 0.1},
        grassDirectionX: {type: 'float', value: 0.0},
        grassDirectionY: {type: 'float', value: 0.0},
...
function draw() {
    ...
    const grassAngleDeepDegree = 2.5; 
    if (grassAngle >= 270) {
        grassDirectionX = (grassAngle - 270) * (1.0 / 90.0) / grassAngleDeepDegree * grassWindSpeed;
        grassDirectionY = (grassAngle - 360) * (1.0 / 90.0) / grassAngleDeepDegree * grassWindSpeed;
    } else if (grassAngle >= 180) {
        grassDirectionX = (grassAngle - 270) * (1.0 / 90.0) / grassAngleDeepDegree * grassWindSpeed;
        grassDirectionY = (180 - grassAngle) * (1.0 / 90.0) / grassAngleDeepDegree * grassWindSpeed;
    } else if (grassAngle >= 90) {
        grassDirectionX = (90 - grassAngle) * (1.0 / 90.0) / grassAngleDeepDegree * grassWindSpeed;
        grassDirectionY = (180 - grassAngle) * (1.0 / 90.0) / grassAngleDeepDegree * grassWindSpeed;
    } else if (grassAngle >= 0) {
        grassDirectionX = (90 - grassAngle) * (1.0 / 90.0) / grassAngleDeepDegree * grassWindSpeed;
        grassDirectionY = (grassAngle - 0.0) * (1.0 / 90.0) / grassAngleDeepDegree * grassWindSpeed;
    } else {
    }
    grassMaterial.uniforms.grassDirectionX.value = grassDirectionX * angleRateNoise;
    grassMaterial.uniforms.grassDirectionY.value = grassDirectionY * angleRateNoise;

    try {
        if (qhyRotationDirection) {
            qhyRotationDirectionChangeAngle += Math.PI / 2 / 100 * dT * 5;
        } else {
            qhyRotationDirectionChangeAngle -= Math.PI / 2 / 100 * dT * 5;
        }
        if (qhyRotationDirectionChangeAngle > (Math.PI / 2 / 100 * 95)) {
            qhyRotationDirection = false;
        } else if (qhyRotationDirectionChangeAngle < (Math.PI / 2 / 100 * 15)) {
            qhyRotationDirection = true;
        }
        // qhyMesh.rotation.y += Math.PI / 2 / 100 * dT * 10;
        qhyMesh.rotation.y = qhyRotationDirectionChangeAngle;
    } catch (e) {
...
function changeGrassDirection(angle = 0.0) {
    grassAngle = angle;
}
...
// must be the last invoked function of javascript
animate();
